...
export default function InterviewPage() {
  ...
  const [company, setCompany] = useState(searchParams.get("company") || "bytedance");
  const [role, setRole] = useState(searchParams.get("role") || "后端开发工程师");
  ...
  const handleStageAdvance = useCallback(
    (nextStage: string) => {
      ...
      const idx = flow.stages.findIndex((s) => s.id === nextStage);
      if (idx >= 0) {
